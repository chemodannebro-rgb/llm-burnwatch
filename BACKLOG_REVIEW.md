...
**Статус на v0.4.1 (после решения владельца проекта «Делаем это» по трём
пунктам P0):** #21 и #2 реализованы полностью как описано выше (см.
`CHANGELOG.md`). #3 реализован частично и осознанно сужен владельцем
проекта: он не хочет скриншот/GIF сейчас, поскольку дашборд будет
переделан — вместо этого добавлена только текстовая строка в README о
том, что команда `dashboard` существует. Собственно скриншот/GIF выделен
в отдельный пункт `#3b` в `BACKLOG.md` (P1, ждёт редизайна дашборда) —
он не потерян, просто больше не блокирует остальные P0.

**Статус на v0.5.0 (P1-пакет по решению владельца проекта «Дальше
делаем»):** #22 (`report` стриминг, скоуп сужен CTO до `report` — `detect`/
`dashboard` вне скоупа), #32 (медиана/MAD один раз на группу), #26
(`report --json`), #13 (`report --trace-id`), #23 (`cached_input_tokens`
как фича), #24 (held-out eval в `train`) и #17 (`hypothesis`
property-based тесты) реализованы полностью, см. `CHANGELOG.md`.
`models/v1` переобучен под новую 4-фичевую форму.

### Итоговый бэклог — единая таблица, упорядоченная по приоритету

| Приоритет | # | Пункт | Владелец(ы) | Статус |
|---|---|---|---|---|
| **P0** | 21 | ~~Гонка между конкурентными `train`/`detect` на одном `model_dir`~~ | Backend/Core Engineer, Security Engineer | **Готово (v0.4.1)** |
| **P0** | 2 | ~~Структурированная объяснимость (поле `reasons`) в `detect --json`~~ | ML Engineer | **Готово (v0.4.1)** |
| **P0** | 3 | ~~Упоминание дашборда в README~~ | Marketing/DevRel, Frontend/UX Engineer | **Частично готово (v0.4.1)** — только текст, без скриншота (см. #3b) |
| **P1** | 3b | Скриншот/GIF дашборда в README (выделено из #3) | Marketing/DevRel, Frontend/UX Engineer | Ждёт редизайна дашборда |
| **P1** | 22 | ~~`report`/`detect` материализуют весь лог перед обработкой~~ | Backend/Core Engineer, CTO | **Готово (v0.5.0, `report` только — см. заметку в BACKLOG.md)** |
| **P1** | 32 | ~~`analyze()` пересчитывает медиану/MAD на запись вместо одного раза на группу~~ | ML Engineer | **Готово (v0.5.0)** |
| **P1** | 26 | ~~У `report` нет флага `--json`~~ | Backend/Core Engineer | **Готово (v0.5.0)** |
| **P1** | 13 | ~~`trace_id` фиксируется, но никогда не читается обратно~~ | Backend/Core Engineer, Директор по продукту | **Готово (v0.5.0)** |
| **P1** | 23 | ~~`cached_input_tokens` не оценивается ни одним из детекторов~~ | ML Engineer | **Готово (v0.5.0)** |
| **P1** | 24 | ~~`train` не печатает метрик оценки, нет отложенной выборки~~ | ML Engineer | **Готово (v0.5.0)** |
| **P1** | 17 | ~~Нет property-based/фаззинг-тестов для статистики аномалий (одобрено: новая dev-only зависимость `hypothesis`)~~ | QA/Test Engineer, CTO | **Готово (v0.5.0)** |
