...
tokens or cost yourself. `CostTracker` has one adapter per provider that
reads usage straight off the response object your own code already has.

**In short:** run `llm-burnwatch init` to detect which SDKs are installed
in your environment and print the matching snippet for you — or copy the
one you need below.
...
**LiteLLM** needs no separate adapter: `litellm.completion(...)` returns a
`ModelResponse` that already normalizes every provider to the same
...
llm-burnwatch — each one reads fields off whatever response object your
own code already holds, at call time. Every adapter also accounts for
that provider's own cache-token billing rules (subset vs. additive
counters), so `cached_input_tokens` always means "billed at the cheaper
cached rate", regardless of provider.

## Missing or stale pricing

If the packaged `pricing.json` is missing a model or has a stale rate,
pass point overrides instead of hand-copying the whole file:
...
`pricing_overrides` merges on top of the packaged defaults; pass
`pricing=` instead if you want to replace the whole table. The two are
mutually exclusive.
...
This is the **only** llm-burnwatch command that ever makes a network
call, and only when given an `http(s)://` URL — a local file path never
touches the network. Only import from a source you trust; see
...
A few things worth knowing about this importer:

- It accepts the raw OTLP JSON export shape (`resourceSpans` →
  `scopeSpans` → `spans`), as a single JSON object, a JSON array of such
  objects, or JSONL — one object per line, which is what an OTel
  Collector's file exporter typically writes.
...
- It's tolerant of both attribute-naming generations the spec has had in
  the wild: current (`gen_ai.request.model`, `gen_ai.usage.input_tokens`/
  `output_tokens`) and older/OpenLLMetry-style
  (`gen_ai.usage.prompt_tokens`/`completion_tokens`).
- It's tolerant of spans with no recognizable `gen_ai.*` attributes at
  all — a real trace export usually has plenty of non-GenAI spans (HTTP
  handlers, DB calls), which are skipped silently rather than treated as
  an error.
- A model missing from `pricing.json` imports at `cost_micros=0` with a
  one-time warning, instead of aborting the whole batch over one
  unrecognized model.
...
`request_id` — lives in `src/llm_burnwatch/schema.json`, also printable
with `llm-burnwatch schema`. This is the source of truth for any
non-Python client (Node.js, Go, ...) writing a compatible log: every
record carries a `schema_version` for future format changes, plus a UTC
`timestamp` (ISO 8601) of when the call happened.

Every record needs a `label` (your own name for the call site — e.g.
`"retrieval"`, `"summarize"`) and a `model` identifier as billed,
alongside `input_tokens`/`output_tokens`/`cost_micros`. An optional
free-form `extra` object lets you attach your own metadata (e.g.
`workflow_id`) without changing the schema.
...
- Want detection to catch runaway loops, cost spikes, or model swaps as
  they happen? See the [Detectors](detectors/baseline.md) pages.
