...
This page says, explicitly, what's covered by llm-burnwatch's semver
commitments — see
[Versioning](https://github.com/chemodannebro-rgb/llm-burnwatch/blob/main/CONTRIBUTING.md#versioning)
in `CONTRIBUTING.md` — and what isn't. If it isn't listed here, it's
internal: it can change shape or disappear in a minor or patch release
...
[Connecting to an existing app](connecting.md#missing-or-stale-pricing)
for when to use which.

| Method | Purpose |
|---|---|
| `log_call(*, label, model, input_tokens, output_tokens, cached_input_tokens=0, cost=None, pricing=None, trace_id=None, request_id=None, **extra)` | Logs one call. Returns the JSONL record dict that was written. |
| `log_openai_response(response, *, label, model=None, trace_id=None, request_id=None, **extra)` | Adapter: reads `response.usage` (OpenAI SDK shape, or an equivalent dict). |
| `log_anthropic_response(response, *, label, model=None, trace_id=None, request_id=None, **extra)` | Adapter: reads `response.usage` (Anthropic SDK shape). |
| `log_gemini_response(response, *, label, model=None, trace_id=None, request_id=None, **extra)` | Adapter: reads `response.usage_metadata` (`google-genai` SDK shape). |
| `log_ollama_response(response, *, label, model=None, trace_id=None, request_id=None, **extra)` | Adapter: reads `prompt_eval_count`/`eval_count` directly off the response — Ollama has no `usage` object. Pass the final streamed chunk, not an intermediate one. |
| `log_langchain_result(result, *, label, model=None, trace_id=None, request_id=None, **extra)` | Adapter: reads `result.usage_metadata` (current LangChain), or falls back to `result.llm_output["token_usage"]` (older `LLMResult`). |
| `guard(*, trace_id=None, max_usd_per_trace=None, max_calls_per_trace=None)` | Context manager. Raises `BudgetExceededError` from the `log_call()`/adapter call that pushes a matching-`trace_id` trace over the given limit. In-process, per-trace enforcement — a different mechanism from `budget`/`BudgetDetector` (cross-process, month-long, post-hoc). See [`budget` vs `guard()`](budget-vs-guard.md). |
...
Each SDK adapter ends by calling `log_call(...)` internally, so any
adapter can raise `BudgetExceededError` the same way `log_call()` does,
if a matching `guard()` block is active.

`request_id` is not the same thing as `trace_id`. `trace_id` groups
several *different* calls into one logical request, for `guard()`'s
budget enforcement. `request_id` is an idempotency key for detecting the
*same* call being logged twice — your own retry after a timeout, for
example. Seeing the same `request_id` again within
`request_id_dedup_window_seconds` (constructor arg, default 300s)
triggers only a warning; the record is still written and the cost is
still counted.

### `BudgetExceededError`

Raised by `guard()` — and, through it, by `log_call()`/every adapter —
when a guarded trace goes over its limit. The call that triggered it has
...
Twelve subcommands (`llm-burnwatch <command> --help` for the full flag
list of any of them — this table covers what most users need):

| Command | Purpose | Key flags | Exit codes |
|---|---|---|---|
| `init` | Detect installed LLM SDK(s) and print a personalized quickstart. | — | `0` |
| `report` | Summarize cost from a log. Defaults to the last 30 days. | `--log-file` (defaults to `default_log_path()`), `--all-time`, `--since`/`--until`, `--trace-id`, `--json`, `--format text\|csv`, `--fx-rate`/`--currency` (`--rub-rate` is the deprecated predecessor) | `0` success, `2` missing log/bad flag combo |
| `dashboard` | Write a static, self-contained HTML cost dashboard. | `--log-file`, `--out` (required), `--since`/`--until`, `--fx-rate`/`--currency` | `0` success, `2` missing log |
...
| `detect` | Run all detectors once against a log, or stream alerts continuously with `--follow`. | `--log-file` (defaults to `default_log_path()`), `--sensitivity low\|normal\|high` (default `normal`; mutually exclusive with the advanced `--threshold`), `--allowed-models`, `--max-call-cost`, `--max-trace-cost`, `--frequency-detector auto\|on\|off`, `--cusum-detector on\|off`, `--json`, `--follow` (+ `--poll-interval`, `--alert-cooldown-minutes`, `--webhook-url`, `--slack-webhook-url`, `--telegram-bot-token`/`--telegram-chat-id`, `--exec-sink`) | `0` no findings, `1` findings (anomalies/rule violations/frequency spikes/level shifts/budget alerts), `2` missing log |
| `status` | Show total spend and which gated detectors (`frequency`/`cusum`/`budget`) are on, off, or learning for a log — without running detection. | `--log-file` (defaults to `default_log_path()`), `--json` | `0` success, `2` missing log |
...
| `budget set` / `budget show` | Configure or inspect the monthly USD budget consulted by `detect`'s `BudgetDetector` and `report`'s budget section. | `set --monthly --warn-at` | `0` |
...
usage error — bad flags, a missing or unreadable file, a missing
optional dependency. An unexpected internal error (a bug, not a usage
mistake) also returns `2`, with a message pointing at the issue tracker
...
optional `[anomaly]` extra and a trained model are both available).

`sensitivity` was added as a purely additive key — per
`alert_schema.json`'s own additive-keys policy, it does not bump
`alert_schema_version`, and no existing key changed meaning.

### `status --json`

`call_count`, `total_cost_usd`, `detectors` (list of `{name, state,
message}` — `state` is one of `on`/`off`/`learning`), `budget_status`
(present only when a monthly budget is configured).
...
- **`detect --follow`'s NDJSON stream** — one alert object per line, the
  same fields as one entry of `detect --json`'s
  `anomalies`/`rule_violations`/etc. arrays.
...
statistics, feature extraction, ML training/registry), `alert_cooldown.py`,
`logreader.py`, `follow_state.py`, `dashboard.py`'s rendering internals,
and any `cli.py`/`tracker.py` function or class not listed above —
including ones without a leading underscore, such as `build_report()`,
